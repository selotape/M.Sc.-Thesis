...
\subsection{Constructing a Reference Model}


Every demographic model $\M$ has a large variety of valid reference models, which we would like to consider. A reference model $\Mref$ may be obtained from the hypothesis model $\M$ via the following \textcolor{red}{three}-step process:
...
\item Then \textbf{collapse the subtree} using one of the structural collapse operations. Gerenarlly speaking, collapsing is the act of merging the populations of a subtree. Here we introduce two types of collapsing; 

Clade-Collapsing is the act of the merging all the populations of a subtree rooted at population $P$ into a single population. This new population contains the portion of the genealogies previously located inside the subtree. We denote the resulting merged population $clade(P)$ (Figure $(\ref{fig:clade_collapse_AB})$). 

Similiarly, Comb-Collapsing is the act of the merging populations of a subtree, while perserving its leaves up to the comb-age. This will be described later.
...
Under this process, any part of the topology which is not transformed is mapped identically onto the reference model. See for example populations $ABC$ and $C$ in figure \ref{fig:clade_collapse_AB}. This allows us, when calculating genealogy likelihoods, to reuse statistics gathered by \gp.
