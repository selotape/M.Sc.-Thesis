...
The computation schema we describe uses a single MCMC, emits statistics in proportion to the model and number of MCMC iterations, and allows free choice on all three axes.  This is showcased in the following expansion of the genealogy likelihood of some sample $G_i$:

\begin{align}
% 
& P(G_i|\{\theta\}_i, \{\tau\}_i, \{\mu\}_i, \M) =~~  \prod_{g_l \in G_i} P(g_l | \{\theta\}_i, \{\tau\}_i, \Tr ) \cdot \prod_{g_l \in G_i} P(g_l | \{\theta\}_i, \{\tau\}_i, \{\mu\}_i, \B )
\label{eq:is_harmonic}
\end{align}


\begin{align}
P(g_l | \{\theta\}_i, \{\tau\}_i, \Tr ) = \prod_{p\in\Tr} \prod_{I\in INT_{g_l}(T)} \frac{2}{\theta_p} \exp{-\frac{(n^2-n)t_I}{\theta_p}}\\
\label{eq:is_harmonic}
\end{align}
...
\begin{align}
P(g_l | \{\theta\}_i, \{\tau\}_i, \{\mu\}_i, \B ) = \prod_{p\in B} \prod_{I\in INT_{g_l}(B)} \frac{2}{\theta_p} \exp{-\frac{(n^2-n)t_I}{\theta_p}}\\
\label{eq:is_harmonic}
\end{align}


Where $INT_{l}(p)$ and $INT_{l}(b)$ are the intervals between coalescence events and migration events accordingly in locus $l$ of $G_i$, and $\tau_I$ is the time span of interval $I$.
