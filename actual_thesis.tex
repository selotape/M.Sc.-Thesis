...
%
One recent successful approach for parameter inference has been to assume the model structure $\M$ is given, then to explicitly represent the genealogy of the sampled individuals at unlinked short loci, and use these local genealogies along-side the target model parameters as hidden variables in a Markov chain Monte Carlo (MCMC) sampling algorithm, to produce Bayesian estimates of the target parameters. [TODO - reference said methods]
%
These methods are advantageous over methods which do not explicitly represent the genealogical relationship between individuals for several reasons; At their core is a full probabilistic generative model of the data. This allows use of more complex model structures. Also, These models effectively integrate out the information on genealogical relationships between individuals, and they produce approximate posterior distributions on model parameters, which quantify the uncertainty of their estimations.\\
%
However, exactly because of these methods conditioning on a given model structure $\M$, there is no straightforward way to use their posterior distribution on parameters to evaluate the assumed model fit.
%
In pronciple, measuring of model fit can be approximated by using importance sampling on the approximated posterior distribution, but standard methods for doing this are statistically inefficient [TODO - reference].
%
