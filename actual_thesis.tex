...
Demography inference methods typically take in sequence data $X$ from a collection of individuals originating from closely related populations and a parameterized demographic model $\M$, and they infer
...
%
Existing demography inference methods, as described in the introduction, don’t directly calculate likelihood - $P(X|\M)$, with or without parameters $\T$. The consequence is that researchers are unable to efficiently test many model hypotheses to explain the sample DNA data and tackle Model Comparison.
%
