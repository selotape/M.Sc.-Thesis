...
Since the denominator $P(\GT^{(i)}|\M)$ is constantly calculated by gphocs in the MCMC process, what remains for mcref to calculate in order to estimate the Bayes Factor is the Model Pairing Conditional $\Pref(\GT^{(i)}|\Mref)$. As examplified in ($\ref{eq:pref_null}$), this is composed of hidden variable likelihoods and the genealogy likelihood over the reference model. The following chapter addresses calculation of the genealogy likelihood efficiently and for multiple reference models, as this represents the bulk of our computational challenge.\\
...
\begin{enumerate}
\item We First "collapse" some sub-tree of the hypothesis. Collapsing is the process of merging the populations of the subtree.  Denote $clade(p)$ as a population which is the result of merging all the populations of the sub-tree rooted at $p$ into a single population. Similiarly $comb(p)$ will denote collapsing the sub-tree rooted at $p$ but perserving it's leaves up to the comb-age.
...
{reuse_statistics}
\caption{When collapsing population $ab$ and identically mapping the rest of the hypothesis onto $\Mref$, we must only recalculate genealogy likelihood inside $clade(ab)$ and may reuse statistics for populations $abc$ and $c$ calculated by gphocs, thus saving valuable effort in calculation of $P(\G|\T,\Mref)$ \textcolor{red}{TODO - Color-code the populations s.t. the reused stats are emphasized (Maybe fadeout the reused pops?). Also, move the arrow exactly in between the hypotheses and add clearer names for the pops}}
\end{figure}
