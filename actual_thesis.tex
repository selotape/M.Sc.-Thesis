...
def recursive_coal_stats(topology_root):
    if is_leaf_population(topology_root):
        ...
        return GPHOCS.EVENT_CHAIN[topology_root.id]

    left_event_chain = recursive_coal_stats(topology_root.left)
    right_event_chain = recursive_coal_stats(topology_root.right)

    current_event_chain = GPHOCS.event_chain(topology_root).append(merge_sort(left_event_chain, right_event_chain))
...
Calculation of summary statistics for the comb reference model is done very similiarly, but with special attention to comb-leaves. Comb-leaves are the populations which remain under the comb after it has been collapes. in every iteration of the reco
