...
%
%
\subsection{Sufficient Statistics for the Reference Genealogy Likelihood $\mathbf{P(G|\Theta, \Mref)}$}
...
Figure 1 illustrates embedding a single local genealogy into the a reference model. Note that the reference model has a distinct set of intervals with different elapsed times and numbers of lineages. Computing the sufficient stats based on these intervals in population $root$  in $\Mnull$ and plugging them into formula (my7+8) would yield the value of $P(\G| \T,\Mnull)$.
