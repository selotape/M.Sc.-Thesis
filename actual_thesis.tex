...
\begin{align}
% 
P(G^{(i)}|\{\theta\}^{(i)}, \{\tau\}^{(i)}, \{\mu\}^{(i)}) =~~ 
\prod_{(i)} \Bigg(&  \prod_{p \in \Tr} \prod_{I\in INT_{G_i}(p)} 
      \bigg(  \Big( \frac{2}{\theta_{p,i}} \Big) \exp \Big\{ -  \frac{(n^2-n)\tau_I}{\theta_{p,i}}   \Big\}  \bigg)  \notag \Bigg)  \cdotp\\ % ~~   
\Bigg(&\prod_{b \in \B} \prod_{I\in INT_{G_i}(mb)} 
      \bigg(  m_{b,i} \exp \Big\{ -  m_{b,i} n~\tau_I   \Big\}  \bigg) \Bigg)  \\ %
\label{eq:is_harmonic}
\end{align}

Formula (2) is a detailed expansion of the estimated genealogy likelihood on samples $G^{(i)}$.
