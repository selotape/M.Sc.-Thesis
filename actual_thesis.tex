...
\subsection{\textcolor{red}{Detached paragraph} Required calculations for equation 9}
...
\subsection{Trade-off between emitting data and ref-model flexibility}
...
%
To fully satisfy the second condition we need to extend the set of statistics to enable calculation of $P(\G|\T,\Mref)$ for reference models that are structurally different from $\M$.
%
We do this, using the theory explained in chapter \textit{\#123\#}, by embedding the local genealogies in the reference model and calculating the summary statistics over the distinct set of intervals in the reference model. 
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]
{embed_in_null_model}
\caption{Embedding a local genealogy in the null reference model}
\end{figure}

Figure 1 illustrates embedding a single local genealogy into the null reference model. Note that the null reference model has a distinct set of intervals with different elapsed times and number of lineages. Computing the sufficient stats based on these intervals in population $root$  in $\Mnull$ and plugging them into formula (my7+8) would yield the value of $P(\G| \T,\Mnull)$.

To satisfy our second requirement, of supporting a wide variety of reference models, the proccess illustrated in Fig 1 must be implemented for a variety of topologies. We achieve this by collapsing every sub-topology into both a clade and a comb reference model and caculating summary statistics over this new sub-topology. This allows us to later compose a valid reference model using different pieces of topology and reuse of stats from the hypothesis model calculated by gphocs.
%
To calculate summary statistics for both reference model types for all sub-topologies in an efficient manner, calculation is done recursively down the population tree, starting from the root population. The $numCoals$ and $numMigs$ of a clade rooted at pop $p$, respectively, are simply the sum of $numCoals$ or $numMigs$ of it's two child clades plus the $numCoals$ or $numMigs$ of population $p$.
%
The $coalStats$ calculation of a clade is based on it's chronological event chain \#TODO - maybe explain what the "event-chain" is?\#. The event-chain of a clade is obtained by merge-sorting the events-chains of it's two child clades, and then appending the event-chain of the clades root population. On this merged event-chain we calculate $coalStats$ as described in \#456\#.
...
\subsection{Omitted Text}
\begin{itemize}

\item \underline{Omit priors in final gene\_ld}
