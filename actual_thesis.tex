...
Formula \ref{eq:gen_ratio_comb} shows how, for a comb reference model, contribution of the genealogy-likelihood to the model-pairing conditional is reduced to contribution of the portion of genealogies above $\tmin$ - 
\[\frac{ P({\G}_{c|>\tmin}|\M_0,\theta_0=\troot) }{ P(\G_{>\tmin}|\M,\T)} ~ .\]

The same canceling-out of terms applies equaly when the reference model is built using a comb-collapse operation on some ancestral population $p$, The difference being that here matter only the portion of genealogies inside the subtree under $p$ and above $\tmin$. We denote this reference model $\M_{\comb p}$ and get the following contribution - 
\[\frac{ P(\G_{c|\in\comb p}|\M_{\comb p},\T_{\comb p},\tmin) }{ P(\G_{\in \comb p}|\M,\T)} \]
As in the case for clade reference models, we wish to calculate statistics which will enable us to calculate the above fraction for all viable comb reference model after only one MCMC chain.
%
To calculated sufficient statistics for all possible combs, $ \{~numCoals(\G,comb(p)),~~ coalStats(\G,comb(p))~\}_p$,~ a recursive technique is applied down the population phylogeny of $M$, similiar to the one defined for clades, the difference being that for combs the function \pythoninline{calculate_coal_stats} takes into account only intervals above $\tmin$.

The denominator, as opposed to the enumerator, does not depend on the reference parameter mapping, rather only on the chosen comb. We calculate this for each ancestral population $p$ by summing the genealogy likelihoods of all populations in the subtree of $p$, and subtracting the genealogy likelihood of  $\G_{<\tmin}$ in the leaves under $p$ (see figure \ref{fig:calculate_hyp_stats_above_tmin}).


\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]
{calculate_hyp_stats_above_tmin}
\caption{Somehow illustrate what $P(\G|\M,\T)$ above $\tmin$ means....}
