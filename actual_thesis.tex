...
$\Ib$ as the set of intervals for migration band $b$, i.e. intervals in the target population of $b$ that also intersect the life span of migration band b. We denote the number of lineages within interval $I$ by $n_I$ and it's length $t_I$. We also define two indicator functions over these intervals, $\1_c(I)$ and $\1_m(I)$, which indicate accordingly whether interval $I$ ends in a coalescence event and whether $I$ ends in migration event. Finally we define the total number of coalescence events across loci in some population $NC(p) := \sum_{G_l\in\mathbf{G}}\sum_{I \in \Ip} \1_c(I)$, and the total number of migration event in some migration band $NM(b):= $
