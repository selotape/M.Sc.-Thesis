...
%
We do this, following the theory explained in chapter \textit{\#123\#}, by embedding the local genealogies in the reference model and calculating the summary statistics over the distict set of intervals in the reference model. The following illustrates embedding a single local genealogy into the null reference model - 
...
Note that the null reference model has a distinct set of intervals with different elapsed times and number of lineages.

To satisfy our second condition over the sufficient statistics, of supporting a wide variety of reference models, we must repeat the proccess illustrated in Fig 1 for any sub-topology we may appear in a reference model (except for those already existant in the hypothesis model, for which we already have sufficient statistics). this means we got's to cumpute dis shit repeatedly on many different sub topologies. Enter optimizations!!


- Clearly state what reference models we support
