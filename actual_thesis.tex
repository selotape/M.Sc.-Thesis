...
\begin{small}
\begin{align}
 \ln\big(P(\mathbf{G}|&\{\theta\}, \{\tau\}, \{\mu\}, \M)\big) ~~~
=~~   \notag \\ 
%
&=~~  ln\big( \prod_{G_l\in \mathbf{G}} P(G_l|\{\theta\}, \{\tau\}, \{\mu\}, \M)\big) \notag \\ 
%
&=~~ ln{\Big(\prod_{G_l \in \mathbf{G}} \prod_{p\in\Tr} \prod_{I \in \Ip} (\frac{2}{\theta_p})^{\1_c(I)} \exp{\Big(-\frac{(n^2-n)~t}{\theta_p}\Big)}  \cdot 
\prod_{G_l \in \mathbf{G}} 
\prod_{b\in B} \prod_{I \in \Ib} m_{b}^{\1_m(I)} ~ \exp{( - m_b~ n~t )} \Big)} \notag \\ 
&=~~ \sum_{p\in\Tr}\Big(NC_{G_l}(p)\ln{(\frac{2}{\theta_p})} - \frac{\sum_{G_l}\sum_{\Ip}(n^2-n)t}{\theta_p}\Big) + \sum_{b\in B} \Big( NM_{G_l}(b)\ln{(m_b)} - m_b\sum_{G_l}\sum_{\Ib}~ n~t \Big) \notag \\ 
\label{wut}
\end{align}
\end{small}
