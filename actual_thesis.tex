...
The denominators (from right to left) are the parameter priors and the genealogy likelihoods, which under the kingman coalescent model depend only on population parameters. They are constantly calculated by gphocs as part of the MCMC flow. The enumerators then are what remains to be calculated for all $\{\GT^{(i)}\}$. 
	
\item \underline{for post-choosing of reference model, calculate all possible ref models}
 
Several considerations led us towards the goal of making the above calculations as generic as possible, enabling choice of multiple reference model based on data from a single MCMC run; By its nature, the use of mcref requires choosing the reference model that is best suited for comparing the competing hypotheses. Every hypothesis being weighted requires a long execution of gphocs. A researcher utilizing mcref may wish to compare between many candidate hypotheses and may wish to obtain comparisons based on many reference models.

Given these considerations, our objective is to output a collection of values for each $G^{(i)}$ which allows to compute $\Pref(\GT^{(i)}|\Mref)$ for as many reference models as possible..
