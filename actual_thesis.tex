...
To accomplish this we wish to express the genealogy likelihood $P(\G|\Theta, M)$ where $\G$ is a series of locus-specific independant genealogies $\G=\{G_1,...G_L\}$. Following kingman coalescent theory, the likelihood of a local genealogy $G_l$ can be expressed through the contribution to likelihood of the time intervals it is composed of. The time intervals of $G_l$ are the parts of the genealogy in a given population $p$ during which the number of lineages is constant. We denote the set of such intervals in genealogy $G_l$ in population $p$ by $\Ip$. Similarly, we denote 
$\Ib$ as the set of intervals for migration band $b$, i.e. intervals in the target population of $b$ that also intersect the life span of migration band b. We denote the number of lineages within interval $I$ by $n_I$ and it's length $t_I$. We also define two indicator functions over these intervals, $\1_c(I)$ and $\1_m(I)$, which indicate accordingly whether interval $I$ ends in a coalescence event and whether $I$ ends in migration event.
...
We have 4 different sums across local genealogies and intervals. We nickname these num-coals $NC(p)$, coal-stats $CS(p)$, num-migs $NM(b)$ and mig-stats $MS(b)$, reaching a final genealogy formula - 
