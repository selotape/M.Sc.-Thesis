...
\item \underline{story}

We want to calculate the Relative bayes factor. Formula (13) is an example for RBF on the null reference model. it's divided into genealogy likelihoods and free parameter likelihoods. Some of the free paraameter likelihoods are the same in hyp and ref. Some are different in ref and dependant on the specific genealogies.

Recall that the genealogy likelihoods are calculated via kingman coalescent. Since we assume short unlinked independent loci, we calculate genealogy likelihood independently per genealogy. To get the over-all genealogy likelihood we simply multiply the independent likelihoods of the specific genealogies. 

We also want the ability to "compose" different reference models in hins-sight. For this we calculate genealogy likelihood seperately for each population.
