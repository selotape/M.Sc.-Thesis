...
In the above equation $P(\T|\M)$ is the prior probability of model parameters to take current values. $P(\G_l|\M,\T)$ is the probability of local genealogy $G_l$ at locus $l$ given the model parameters, calculated under the Kingman Coalescent model, with special regard to migration events, and $P(\X_l|\G_l)$ is the local data likelihood given local genealogy $G_l$, which is computed using standard DNA substitution models (\cite{JUKECANT69}).
%
In each MCMC update step \gp proposes a new instace of $\G, \T$ and decides if to accept the proposal based on the ratio between complete likelihoods of the current instance and proposed instance. 
[TODO - add 3 sentences explaining gphocs update steps]
...
\item Update genealogy structure: For each subtree of each genealogy, alter the subtree using a subtree-pruneand-regraft operation. 
...
\item Rescaling: Slightly perturb all model parameters ${\theta_p}$, ${\tau_p}$ and all coalescent
...
\caption{\gp uses distinct independant loci, chosen to be far away from genes to reduce the influence of selection. Over each loci a local genealogy is represented and embedded in the population phylogeny.}
...
The model selection problem takes a sequence data set and a series of phylogeny models $\M_1,... \M_n$, which differ in their structural components, and aims to find the one which best fits the data set, i.e. select the model $\M_i$ with maximal $P(X|\M_i)$. Typically only the structural assumptions of the models are compared ($\Tr$ and $B$), and not specific parameter values ($\T$). [TODO - extend figure with more examples (see caption)]
...
\captionsetup{width=.8\textwidth}
\caption{An example problem of selecting between three model with different structures. In model 1 populations $A$ and $B$ are siblings. In model 2 populations $B$ and $C$ are siblings and there exists a gene-flow from $A$ to $B$, and in model 3 only to leaf populations exist - $AB$ and $C$. }
...
\centering
\includegraphics[width=0.8\textwidth]
{null_reference_model}
\captionsetup{width=.8\textwidth}
\caption{A mapping from hypothesis model $\M_{hyp}$ onto the null reference model $\M_0$. Population sizes $\{\theta_{p_a}, \theta_{p_b}, \theta_{p_c}, \theta_{p_{ab}}\}$, as well as the migration rate $\{mb_{b\leftarrow c}\}$ are mapped according to their prior probability in $\M_{hyp}$. These have no effect on the reference model structure. Divergence times $\{\tau_{ab}, \tau_{root}\}$ are mapped onto a uniform distribution with upper bound calculated as described in appendix \ref{ap:cond_mig}. The remaining parameter $\theta_{root}$ can be freely mapped. The remaining parameter $\theta_{root}$ can be freely mapped onto $\theta_{p_0}$ so as to minimize the RBF ratio. [TODO - redraw the migrating lineage so as to differentiate between the migration and coalescence events.]}
\label{fig:null_reference_model}
\end{figure}
%
...
\captionsetup{width=.8\textwidth}
\caption{A single local genealogy mapped into the comb reference model. The comb model consists of a single ancestral population $p_{comb}$ [TODO - use $p$ with the comb symbol instead of $p_{comb}$ ] which splits simultaneously into leaves $L$. [TODO - rewrite similiarly to figure \ref{fig:null_reference_model}]}
...
%
In such cases, we wish to consider a reference model where only a subset of the sampled populations $P$ is collapsed into a single clade population, or a comb submodel. The null and comb models discussed in sections 3.4-3.6 are thus special cases where $P$ is the entire set of sampled populations.\\
%
...
Having defined reference models and formulated their relative Bayes factors, we now describe the computational scheme we use to estimate relative Bayes factors as derived in subsection \ref{Relative Bayes factors}:
%
\begin{equation}
...
The right hand denominator of formula \ref{eq:computational_scheme}, $P(\GT^{(i)}|\M)$, is constantly calculated by \gp in the MCMC process. What remains for McRef to calculate in order to estimate RBF is the model pairing conditional $\Pref(\GT^{(i)}|\Mref)$. In the following section we address efficient calculation of genealogy likelihood in the reference model, since it is the main component making up the model pairing conditional and represents the bulk of our computational challenge.

\subsection{Phases of computation}

We perform the RBF calculation in two phases, so as to optimize the balance between execution time and algorithm flexibility. 
%
Phase 1 computes summary statistics which aggregate most genealogy statistics required to calculate genealogy likelihood in the reference model - $P(G|\Theta, \M_{ref})$.
%
This postpones choice of mapping of free parameters to a later phase. Subsection \ref{Sufficient Statistics for Reference Model Genealogy Likelihood} explains how this is achieved and why.
%
This phase is performed jointly with the MCMC sampling process.
%
The output of this phase allows calculation of RBF for every viable reference model. This postpones choice of reference model structure to a later phase. Subsections \ref{Recursive Calculation of Sufficient Statistics for Multiple Clade Reference Models} and \ref{Calculating Comb Reference Model Genealogy Likelihood} explain how this is efficiently performed.
%
Phase 2 finalizes the RBF calculation. Given a directive of reference model and  mapping of it's free-parameters, it plugs in the appropriate parameter priors and emits the final estimate of $ \frac{1}{\rbf(\M:\Mref|\X)}$. Subsection 4.5 explains how this is performed.
%
By executing phases 1 \& 2 for each competing hypotheses, with the same choice of reference model in phase 2, we get comparable estimates of relative Bayes factors, indicating which hypothesis has the best fit.

\subsection{Efficient Sufficient Statistics for Reference Model Genealogy Likelihood}
\label{Sufficient Statistics for Reference Model Genealogy Likelihood}

One objective of our computational scheme is to allow maximal flexibility in choice of reference model, without degregading algorithm run time or space usage.
%
Since the most time consuming step is the MCMC sampling algorithm, we assume and require only a single MCMC chain per hypothesis model.
%
Assuming this, we note that there exists a clear trade-off between the flexibility in choice of reference model and the amount of data the MCMC process is required to emit. 
%
For example, if no flexibility is required and the reference model is predetermined before MCMC execution, formula \ref{eq:rbf} can be calculated during MCMC iteration and only the final RBF estimation need be emitted. 
%
Unfortunately, calculating \ref{eq:rbf} on another reference model would require another full MCMC run.
%
...
Our computational scheme aims to find a reasonable middle ground between these two extreme options.
...
\label{Recursive Calculation of Sufficient Statistics for Multiple Clade Reference Models}
...
\label{Calculating Comb Reference Model Genealogy Likelihood}
...
\subsection{Finalizing the RBF calculation for a chosen reference models}

TODO - write how McRef finalizes the RBF calculation.
