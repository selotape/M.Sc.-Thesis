...
\subsection{Experiment - Population Separation}
In this experiment we generated sets where population $ABC$s divergence (the divergence time between $C$ and the $AB$) is fixed to 0.003, and perturbed $AB$s divergence time ( the divergence time between $A$ and $B$) between 0 to 0.00050. No migration was allowed between any population.

We considered 2 hypotheses:
\begin{enumerate}
\item $\M_{3pops}$ - A model with 3 leaf populations $A$, $B$ and $C$ (the true model used to generate the sequence data)
\item $\M_{2pops}$ - A model with 2 leaf populations $AB$ and $C$, where the sequenced individuals of $A$ and $B$ are grouped in a single population (the true model only when $tau_AB:=0$)
\end{enumerate}
