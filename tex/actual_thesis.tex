...
A main objective of our computational scheme is allowing maximal flexibility in choice of reference model, without degregading algorithm run time or space usage.
%
...
%
Phase 1, which is performed jointly with the MCMC sampling process, emits intermediate summary statistics which meet the above three conditions. Phase 2 is then given a directive of specific reference model structure and mapping of free parameters. This phase assembles the relevant statistics, plugs in the appropriate parameter priors and emits the final estimate of $ \frac{1}{\rbf(\M:\Mref|\X)}$. Phase 2 is performed in post-processing and can be rerun for any reference model, using the same sufficient statistics emitted by phase 1.
%
Subsection \ref{Sufficient Statistics for Reference Model Genealogy Likelihood} explains how conditions 2 \& 3 are achieved for a single model structure.
%
Subsections \ref{Recursive Calculation of Sufficient Statistics for Multiple Clade Reference Models} and \ref{Calculating Comb Reference Model Genealogy Likelihood} explain how this is efficiently extended to multiple reference model structure.
%
Subsection \ref{Finalizing the RBF calculation for a chosen reference models} explains how the intermediate sufficient statistics are finalized into an RBF of a specific reference model.
...
To efficiently achieve this, calculation of $numCoals$ and $coalStats$ is done recursively down the population phylogeny of $M$ (see pseudo-python code below). This is done using a function for computing $coalStats$ given a sorted list of intervals (\pythoninline{calculate_coal_stats}), as well as accessors to data from \gp (\pythoninline{num_coals_from_gphocs} \& \pythoninline{sorted_intervals_from_gphocs}):
%
...
\label{Finalizing the RBF calculation for a chosen reference models}
