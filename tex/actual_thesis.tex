...
Similiarly to experiment I, we used these techniques to compare the true model $\M_{AB_C}$ against the alternatives $\M_{A\_BC}$ and $\M_{AC\_B}$. Figures \ref{fig:results-M4-divAB-comb_ABC} through \ref{fig:results-M4-divAB-HM} show the results of the comparisons.
...
\subsubsection{Observations}
Following is a series of observations drawn from figures \ref{fig:results-M4-divAB-comb_ABC} through \ref{fig:results-M4-divAB-HM} - 

\begin{itemize}
\item The two comb reference methods clearly and confidently choose the true hypothesis model.

\item When using the $Clade(ABC)$ reference model we see a gentle upward trend in results, but no reproducable clear selection. In the remaining two experiments - using the $Clade(ROOT)$ reference model and the harmonic mean, we see absolutely no selection and a high uncertainty.

\item The comb reference methods also correctly showed no preference to any model when the hypotheses were eqivalent. This is not true for the other methods.

\item Amongst the two comb reference methods, the more localized $Comb(ABC)$ provided a stronger and more confident signal.

\end{itemize}

To summarize, we demonstrate that the choice of reference model has a great impact on algorithm performance. The more the reference model was localized to the disputed hypothesis region, the better the algorithm performed.
