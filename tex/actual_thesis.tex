...
We designed a series of experiments in order to test the selection power of our algorithm, and to assess the certainty of an McRef test. 
%
The selection power is embodied in both the type of scenarios in which McRef clearly picks the correct hypothesis, and in the sensitivity of the algorithm, e.g. when parameters converge.
%
A minor goal of the experiment design is to learn how best to employ McRef and how to choose a reference model for a given set of hypotheses.

\subsection{General Setup}
%
We generated data sets under different demographic scenarios, using the following setup constant - 3 leaf populations $A$, $B$ and $C$, an ancestor populations $AB$ and a root ancestor $ABC$. 4 haploid sequences in each leaf population and 5000 loci of length 1000 in each sequence. 
%
...
%
Comparison was done using both the harmonic mean and relative Bayes factors with some reference model $\M_{ref}$. 
%
...
%
For the max value we recorded $max+ste$ and for the min value we recorded $min-ste$. Since the these ste's correspond to the difference between two values, we took the sqrt of the sum of the two appropriate ste's. 
%
As a result we attained 4 values for each comparison of $M_1$ and $M_2$ on a given data set and each method of comparison (e.g. $HM$, $RBF$ with null model). To this we add the fact that each data set has a "sister" set generated with identical model parameters, providing additional indication for noise.


\subsection{Experiment I - Varying Population Separation}
\subsubsection{Setup}
In this experiment we generated sets where population $ABC$s divergence (the divergence time between $C$ and the $AB$) is fixed to 0.003, and perturbed $AB$s divergence time ( the divergence time between $A$ and $B$) between 0 to 0.00050. No migration was allowed between any population. We considered 2 hypotheses:
%
\begin{enumerate}
\item $\M_{3pops}$ - A model with 3 leaf populations $A$, $B$ and $C$ (the true model used to generate the sequence data)
\item $\M_{2pops}$ - A model with 2 leaf populations $AB$ and $C$, where the sequenced individuals of $A$ and $B$ are grouped in a single population (the true model in the data set where $\tau_{AB}:=0$)
\end{enumerate}
%
and compared these two models using each of three options:
%
\begin{enumerate}
\item The harmonic mean
\item Relative Bayes factors where the reference model is $\M_{null}$
\item Relative Bayes factors where the reference model is $\M_{clade(AB)}$ (a clade rooted at population $AB$). Note that when $\Mhyp = \M_{2pops}$ we get $\Mref=\Mhyp$.
\end{enumerate}

We conducted a comparison between $\M_{3pops}$ and $\M_{2pops}$ (i.e., $\log{\frac{P(X|\M_{3pops})}{P(X|\M_{2pops})}])}$ using the three ways mentioned above. Figure \ref{fig:results-divAB} shows for each comparison the 4 values associated with it for each comparison method. Data sets are marked by $div-X-a$, where $X := \tau_{AB} \times 10,000$ and $a$ indicates the data replicate (1 or 2).
...
\caption{[TODO - figure out how to center figure]}
\label{fig:results-divAB}
\end{figure}


\subsubsection{Observations}
Following is a series of direct observations about figure \ref{fig:results-divAB} - 

\begin{itemize}
\item We get values near 0 ($<1e^{-6}$) when  I computing $RBF(\Mhyp=\M_{2pops}, \Mref=\M_{clade(AB)}$, because the reference and hypothesis models converge to the same model. We consider this a simple validation of our RBF calculation.
...
\item When we use $\Mref = clade(AB)$, the estimates of RBF are much less noisy than when using $\Mref = \M_{null}$.
%
...
\subsection{Experiment II - Varying Gene Flow}
\subsubsection{Setup}
In this experiment we generated data sets where the divergence times are fixed to $\tau_{ABC} = 0.00300$, and $\tau_{AB} = 0.00150$ and simulated migration from population $C$ to population $B$ in different rates.
%
We considered four hypothesis models ($\Mhyp$):
\begin{enumerate}

\item $\M_{nomig}$ - A model with no migration bands
\item $\M_{migCB}$ - A model with a migration band from $C$ to $B$ (the true model)
\item $\M_{migALL}$ - A model with migration bands between all pairs of sampled populations (6 mig bands total)
...
\begin{enumerate}
\item Using the harmonic mean (HM)
\item Using RBF where $\Mref = \M_{null}$

\end{enumerate}

To present the results, we conducted a comparison between each of the three models with migration against $\M_{nomig}$ as a base model (e.g. $\log{ \frac{P(X|\M_{migBC})}{P(X|\M_{nomig})}}$). We ploted for each data set and each model with migration the 4 values associated with this comparison, under $null\_RBF$ (Figure \ref{fig:results-migCB-1}) and HM (Figure \ref{fig:results-migCB-2}). Data sets are marked by $mig-X-a$, where $X$ indicates percentage of admixture in population $B$ and $a$ indicates the data replicate (1 or 2).
...
\subsubsection{Observations}
Following is a series of direct observations about figures \ref{fig:results-migCB-1} and \ref{fig:results-migCB-2} - 


\begin{itemize}

\item We see that the harmonic mean scores the three models with migrations similarly and it never significantly prefers models with migration to $\M_{nomig}$. In mig10-2 it even significantly prefers the model with no migration.
