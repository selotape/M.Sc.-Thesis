...
%
Model parameters are scaled by mutation rate.
%
...
A common approach to for inferring parameters of a demographic model $\M$ is to assume the model structure $(\Tr,B)$, and to explicitly represent the genealogy of the sequenced individuals at short unlinked loci.
%
...
%
However, because these methods condition on a given model structure $\M$, they provide no straightforward way to compare demographic model hypotheses.
%
...
%
Section 4 explains in depth our implementation of McRef, our model selection algorithm which uses the \gp parameter-inference framework. McRef earned it's nickname due to it's employment of reference models in the MCMC process.
%
...
\subsection{Bayesian inference and \gp}
